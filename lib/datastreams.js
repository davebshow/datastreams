...
DataStream.prototype.simpleStream = function(opts) {
...
DataStream.prototype.linkedStreamToObject = function(opts) {
...
DataStream.prototype.linkedStreamToStream = function(dataStream, opts) {
    ...
    var transformer = new streams.StreamToStreamTransform()
    var reader = db.createReadStream({start: streamStart, end: streamEnd})
    var source = new streams.LinkedStreamToStream(db, reader, stream)
...
function buildUniques(uniqueArray) {
    if(typeof uniqueArray == 'string') return uniqueArray
    var stream = ''
    for (var i in uniqueArray) {
        var key = uniqueArray[i]
